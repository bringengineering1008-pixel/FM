...
class BringIssueTwentyPostImageTests(unittest.TestCase):
    def test_thumbnail_copy_uses_editorially_approved_two_lines(self):
        from scripts.prepare_bringissue_twenty_post_batch import (
            load_registry,
            thumbnail_scene_number,
            thumbnail_lines,
        )

        posts = load_registry()["posts"]
        self.assertEqual(thumbnail_lines(posts[0]), tuple(posts[0]["thumbnail_text"]))
        self.assertEqual(thumbnail_scene_number(posts[0]), 6)
        self.assertEqual(thumbnail_scene_number(posts[1]), 3)
...
class BringIssueTwentyPostDraftTests(unittest.TestCase):
    def test_drafts_match_approved_mobile_editorial_style(self):
        ...
        for post in load_registry()["posts"]:
            ...
            self.assertEqual(draft.count("![장면"), 9, post["slug"])
