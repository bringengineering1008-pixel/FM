...
def build():
    ...
    add_callout(doc, "2026년 9월 운영 목표", "9월 정산 월 50만 원. 모든 글은 유입 역할·수익 연결 방식·예상 정산 시점을 기록하며, 조회수만 쌓고 수익 경로가 없는 편성은 피한다.", PALE_YELLOW)
    ...
    add_para(doc, "브링이슈의 기본 이미지 구성은 실제 자료 50%·제미나이 이해 이미지 30%·자체 정보 카드 20%입니다. 영상형은 유튜브 고화질 캡처를, 일반 정보형은 권리가 확인된 실제 사진과 공식 화면을 증거로 사용합니다. 공개 자료에 없는 배경 상황·감정·맥락은 제미나이의 한국 매거진 에디토리얼 일러스트로 시각화합니다. 일정·가격·노선처럼 표가 더 빠른 정보에만 자체 카드를 제한적으로 사용합니다.")
    add_callout(doc, "제미나이 제작 경로 — 고정", "외부 제미나이 생성 사이트·대행 서비스·외부 API는 사용하지 않는다. 로그인된 Google AI Studio 웹 또는 Google Flow 웹에서만 제작한다. Google AI Studio가 API 키·별도 결제를 요구하거나 사용할 수 없으면 로그인된 Google Flow 웹으로 전환한다. 두 화면 모두 사람 확인(CAPTCHA)이 나타나면 작업을 멈추고 사용자에게 직접 확인을 요청한다.", PALE_GREEN)
    add_callout(doc, "고정 교차 편집 순서", "실제 사진·공식 화면·유튜브 캡처 → 보이는 사실 설명 → 제미나이 이해 이미지 → 편집자 해설 → 필요한 정보 카드", PALE_YELLOW)
    ...
    add_checklist(doc, [
        ...
        "이미지 안에 제목·긴 문장·말풍선·브랜드 로고·유튜브 UI를 넣지 않는다. Gemini가 자동으로 넣은 고유 출처 표시는 제거하거나 가리지 않는다.",
        "AI 이미지를 실제 사건의 증거처럼 배치하지 않고 연속 두 장 이상 사용하지 않는다.",
        "제미나이 이미지를 연속으로 배치하지 않고, 앞이나 뒤에 실제 영상 캡처와 설명 문단을 둔다.",
        "모든 AI 이미지 아래에 ‘AI로 제작한 이해용 이미지입니다.’라고 표시한다.",
        "모든 실제 캡처 아래에 ‘영상 캡처: 원본 YouTube 영상 (화면 UI 제거·내용 확인용)’이라고 표시한다.",
        "생성 결과에 가짜 글자·숫자, 요청하지 않은 인물, 브랜드 로고, 깨진 손이 있으면 그대로 쓰지 않고 재생성한다.",
        "Flow 결과에 실제 앱 UI·유사 유튜브 화면이 생기면 편집 프롬프트로 화면을 추상 그래픽으로 교체한 뒤 다시 검수한다.",
    ])
    add_callout(doc, "제미나이 공통 프롬프트", "한국 온라인 매거진에 들어갈 16:9 가로형 에디토리얼 일러스트.\n\n장면: [누가 / 어디서 / 무엇을 하는지]\n핵심 감정: [놀람 / 배려 / 긴장 / 반전 / 따뜻함]\n주요 소품: [문단을 설명하는 물건 1~3개]\n구도: [와이드숏 / 미디엄숏 / 손과 소품 중심 / 공간 중심]\n색감: 밝고 세련된 크림·옐로 / 블루 / 피치 계열.\n스타일: 고급 라이프스타일 매거진 삽화, 자연스러운 빛, 충분한 여백, 현실적인 공간감, 섬세한 질감.\n\n실제 보도사진처럼 만들지 말 것. 실존 인물의 얼굴을 그대로 복제하지 말 것. 읽을 수 있는 글자·숫자, 자막, 브랜드 로고, 말풍선, 실제 앱 UI를 넣지 말 것. Gemini 고유 출처 표시는 제거하지 말 것. 해상도 1200×675 이상.", PALE_YELLOW)
    add_heading(doc, "6.4 일반 정보형 하이브리드 예시", 2)
    add_callout(doc, "추석 기차표 글", "실제 KTX 사진 → 통합회원 설명 → 가족의 예매 준비 제미나이 이미지 → 노선별 공식 일정 카드 → 실제 KTX 사진 → 예매 직전 제미나이 이미지 → 결제 일정 카드 → 결제일 확인 제미나이 이미지", PALE_GREEN)
    add_para(doc, "실제 사진은 열차와 이동의 현장감을, 공식 자료 기반 카드는 날짜와 노선을, 제미나이 이미지는 가족이 준비하고 기다리고 확인하는 행동을 설명합니다. AI 이미지가 일정이나 예매 성공의 증거처럼 보이지 않도록 반드시 역할을 분리합니다.")
    doc.add_page_break()
    add_heading(doc, "6.5 짧은 녹화·GIF 기준", 2)
    ...
    steps = [
        ...
        "실제 자료 50%·제미나이 이해 이미지 30%·필수 정보 카드 20%를 기본으로 하되 글의 성격에 맞게 조정한다.",
        "모든 제미나이 이미지 아래에 ‘AI로 제작한 이해용 이미지입니다.’ 캡션을 입력한다.",
        "실제 캡처와 제미나이 이미지를 교차 배치하고 각 이미지 아래에 서로 다른 출처·AI 고지를 입력한다.",
    ...
    add_checklist(doc, [
        ...
        "실제 영상 캡처가 각 핵심 구간의 증거로 배치되고 ‘영상 캡처: 원본 YouTube 영상’ 표기가 있다.",
        "제미나이 이미지를 연속으로 배치하지 않았고 실제 캡처와 해설 문단 사이에 교차 배치했다.",
